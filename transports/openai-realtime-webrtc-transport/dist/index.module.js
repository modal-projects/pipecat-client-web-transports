import {RTVIError as $4c8Bm$RTVIError, logger as $4c8Bm$logger, RTVIMessageType as $4c8Bm$RTVIMessageType, TransportStartError as $4c8Bm$TransportStartError, RTVIMessage as $4c8Bm$RTVIMessage, Transport as $4c8Bm$Transport} from "@pipecat-ai/client-js";
import $4c8Bm$dailycodailyjs from "@daily-co/daily-js";
import {dequal as $4c8Bm$dequal} from "dequal";
...
$parcel$export($4e887b6dcd7ec3e7$exports, "OpenAIRealTimeWebRTCTransport", () => $4e887b6dcd7ec3e7$export$60a765086a8f0478);



const $4e887b6dcd7ec3e7$var$BASE_URL = "https://api.openai.com/v1/realtime";
const $4e887b6dcd7ec3e7$var$MODEL = "gpt-4o-realtime-preview-2024-12-17";
class $4e887b6dcd7ec3e7$export$60a765086a8f0478 extends (0, $4c8Bm$Transport) {
    ...
    initialize(options, messageHandler) {
        ...
        const existingInstance = (0, $4c8Bm$dailycodailyjs).getCallInstance();
        if (existingInstance) this._daily = existingInstance;
        else {
            this._daily = (0, $4c8Bm$dailycodailyjs).createCallObject({
                // Default is cam off
    ...
    async initDevices() {
        if (!this._daily) throw new (0, $4c8Bm$RTVIError)("Transport instance not initialized");
    ...
    /**********************************/ /** Call Lifecycle functionality */ _validateConnectionParams(connectParams) {
        ...
        if (typeof connectParams !== "object") throw new (0, $4c8Bm$RTVIError)("Invalid connection parameters");
        return connectParams;
    }
    async _connect() {
        if (!this._openai_cxn) {
            (0, $4c8Bm$logger).error("connectLLM called before the webrtc connection is initialized. Be sure to call initializeLLM() first.");
            return;
        }
    ...
    /**********************************/ /** OpenAI-specific functionality */ updateSettings(settings) {
        if (settings.voice && this._channelReady()) {
            (0, $4c8Bm$logger).warn("changing voice settings after session start is not supported");
        ...
        if ((0, $4c8Bm$dequal)(newSettings, this._service_options.settings)) return;
        this._service_options.settings = {
            ...this._service_options.settings,
            ...settings
        };
    ...
    enableScreenShare(enable) {
        (0, $4c8Bm$logger).error("startScreenShare not implemented for OpenAIRealTimeWebRTCTransport");
        throw new Error("Not implemented");
    }
    get isSharingScreen() {
        (0, $4c8Bm$logger).error("isSharingScreen not implemented for OpenAIRealTimeWebRTCTransport");
        return false;
    }
    tracks() {
    ...
    /**********************************/ /** Bot communication */ async sendReadyMessage() {
        ...
        try {
            await p;
            this._onMessage({
                type: (0, $4c8Bm$RTVIMessageType).BOT_READY,
                data: {
                    version: "1.0.0"
                }
            });
        } catch (e) {
            (0, $4c8Bm$logger).error("Failed to start bot");
            throw new (0, $4c8Bm$TransportStartError)();
        }
    }
    sendMessage(message) {
        switch(message.type){
            case (0, $4c8Bm$RTVIMessageType).APPEND_TO_CONTEXT:
                {
            ...
            case "run":
                this._run();
                break;
            case (0, $4c8Bm$RTVIMessageType).LLM_FUNCTION_CALL_RESULT:
    ...
    /**********************************/ /** Private methods */ async _connectLLM() {
        ...
        if (!audioSender) {
            ...
            if (!micTrack) try {
                ...
                (0, $4c8Bm$logger).error("Failed to get mic track. OpenAI requires audio on initial connection.", e);
                throw new (0, $4c8Bm$RTVIError)("Failed to get mic track. OpenAI requires audio on initial connection.");
            }
    ...
    _attachLLMListeners() {
        if (!this._openai_cxn) {
            (0, $4c8Bm$logger).error("_attachLLMListeners called before the websocket is initialized. Be sure to call initializeLLM() first.");
            return;
        }
        this._openai_cxn.ontrack = (e)=>{
            (0, $4c8Bm$logger).debug("[openai] got track from openai", e);
            this._botTracks[e.track.kind] = e.track;
            this._callbacks.onTrackStarted?.(e.track, $4e887b6dcd7ec3e7$var$botParticipant());
        };
        // Set up data channel for sending and receiving events
        if (this._openai_channel) {
            (0, $4c8Bm$logger).warn('closing existing data channel "oai-events"');
        ...
        this._openai_cxn.onconnectionstatechange = (e)=>{
            ...
            (0, $4c8Bm$logger).debug(`connection state changed to ${state.toUpperCase()}`);
            switch(state){
                case "closed":
                case "failed":
                    this.state = "error";
                    ...
                    } else this._callbacks.onError?.((0, $4c8Bm$RTVIMessage).error(`Connection to OpenAI ${state}`, true));
                    break;
            }
        };
        this._openai_cxn.onicecandidateerror = (e)=>{
            (0, $4c8Bm$logger).error("ice candidate error", e);
        };
    }
    async _negotiateConnection() {
        ...
        if (!apiKey) {
            (0, $4c8Bm$logger).error("!!! No API key provided in service_options");
            return;
        }
        try {
            ...
            (0, $4c8Bm$logger).error(msg);
            this.state = "error";
            throw new (0, $4c8Bm$TransportStartError)(msg);
        }
    }
    _cleanup() {
        this._openai_channel?.close();
    ...
    _updateSession() {
        ...
        (0, $4c8Bm$logger).debug("updating session", session_config);
    ...
    async _handleOpenAIMessage(msg) {
        ...
        switch(type){
            case "error":
                (0, $4c8Bm$logger).warn("openai error", msg);
                break;
            ...
            case "response.function_call_arguments.done":
                {
                    ...
                    this._onMessage({
                        type: (0, $4c8Bm$RTVIMessageType).LLM_FUNCTION_CALL,
                        data: data
                    });
                }
                break;
            ...
            default:
                (0, $4c8Bm$logger).debug("ignoring openai message", msg);
        }
    }
    async _handleTrackStarted(ev) {
        ...
        this._callbacks.onTrackStarted?.(ev.track, ev.participant ? $4e887b6dcd7ec3e7$var$dailyParticipantToParticipant(ev.participant) : undefined);
    }
    async _handleTrackStopped(ev) {
        this._callbacks.onTrackStopped?.(ev.track, ev.participant ? $4e887b6dcd7ec3e7$var$dailyParticipantToParticipant(ev.participant) : undefined);
    }
...
/**********************************/ /** Daily helper functions for device handling */ const $4e887b6dcd7ec3e7$var$dailyParticipantToParticipant = (p)=>({
        id: p.user_id,
        local: p.local,
        name: p.user_name
    });
const $4e887b6dcd7ec3e7$var$botParticipant = ()=>({
        id: "bot",
        local: false,
        name: "Bot"
    });
...
export {$4e887b6dcd7ec3e7$export$60a765086a8f0478 as OpenAIRealTimeWebRTCTransport};
